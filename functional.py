...
def find_char():
    ...
    print()
    char = input('выберите, что делать?  ')
    print()
    ...
    #     # print('0 - id, 1 - second name, 2 - first name, 3 - third name, 4 - number, q - quit')
        char = input("выберите, что делать? ")
    if char != 'q':
        if char == "0" : inp = input('Введите Id абонента - ')
        elif char == "1": inp = input('Введите Фамилию абонента - ')
...
def check_id_record(file_name: str, text: str):
    decision = input(f'Удаляем абонента? 1 - да, 2 - нет, q - выход\n')
...
def change_records(file_name: str):
    ...
    if record_id != 'q':
        replaced_line = f'{int(record_id)} ' + ' '.join(
            input('Введите новые данные абонента \n').split()[:4]) + ';\n'   
